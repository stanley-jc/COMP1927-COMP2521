...
void BSTreeLevelOrder(BSTree t)
{
	...
	if(t != NULL){
		//initialise queue
		Queue treeQueue = newQueue();
		BSTree cur;

		//add T's root node to queue
		QueueJoin(treeQueue,t);
		//the queue still has some entries
		while(!QueueIsEmpty(treeQueue)){
			//take the head of the queue
			cur = QueueLeave(treeQueue);
			//print the value from its BSTree node
			printf("%d ",cur->value);
			//add the left child (if any) to the queue
			if(cur->left != NULL){
				QueueJoin(treeQueue,cur->left);
			}
			//add the right child (if any) to the queue
			if(cur->right != NULL){
				QueueJoin(treeQueue,cur->right);
			}
		}
	}
}
...
int BSTreeNumLeaves(BSTree t)
{
	...
	if(t->left == NULL && t->right == NULL){
		...
		nleaves = BSTreeNumLeaves(t->left)+BSTreeNumLeaves(t->right);
	}
